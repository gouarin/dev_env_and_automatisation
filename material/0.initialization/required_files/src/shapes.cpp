...
Circle
    ...
    ::Circle(const xt::xtensor_fixed<double, xt::xshape<2>> &center,
             double radius, std::size_t npoints)
    : theta(xt::linspace<double>(0., 2. * xt::numeric_constants<double>::PI,
                                 npoints)),
      path(xt::empty<double

                     >

           (std::array<std::size_t, 2>{npoints, 2})) {
